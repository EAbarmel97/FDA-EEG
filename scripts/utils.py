...
def convert_raweeg_2fd(
    ...
    if save_plot:
        fd_basis.plot()
        plt.savefig(f"/storage5/eabarmel/FDA-EEG/plots/raw2fd/{eeg_data.time_period.value}/{eeg_data.time_period.value}_subject{eeg_data.subject_id}.png")
        plt.close()
...
def eeg_fpca(
    ...
    #computes truncated fpca
    fd_basis = convert_raweeg_2fd(eeg_data, n_basis, save_plot=save_plot)
    fpca = FPCA(n_basis)
    fpca.fit(fd_basis)
    ...
    if save_plot:
        ...
        if eeg_data.time_period.value == "before":
            plt.savefig(f"/storage5/eabarmel/FDA-EEG/plots/fpca_components/before/before_subject{eeg_data.subject_id}.png")
            plt.close()
        else:
            plt.savefig(f"/storage5/eabarmel/FDA-EEG/plots/fpca_components/after/after_subject{eeg_data.subject_id}.png")
            plt.close()

    eigvals = np.square(fpca.singular_values_)
    eigvals[np.abs(eigvals) >= np.finfo(float).eps]
